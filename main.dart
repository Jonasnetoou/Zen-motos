...
// -----------------------------------------------------------------------
// FUNÇÕES GLOBAIS (usadas tanto na tela principal quanto no histórico)
// -----------------------------------------------------------------------
String comFallback(String valor) {
  final limpo = valor.trim();
  return limpo.isEmpty ? 'NÃO INFORMADO' : limpo;
}

String formatarQtdGlobal(double qtd) {
...
String valorFmtGlobal(double valor) {
...
String gerarTextoWhatsApp({
  ...
  final clienteFmt = comFallback(cliente);
  final total = itens.fold(0.0, (soma, item) => soma + item.valorTotal);
  ...
  for (final item in itens) {
    final qtd = formatarQtdGlobal(item.quantidade);
    buffer.writeln(
        'CLIENTE: $clienteFmt | ${qtd}x ${item.descricao} | R\$ ${valorFmtGlobal(item.valorTotal)}');
  }

  buffer.writeln();
  buffer.writeln('TOTAL: R\$ ${valorFmtGlobal(total)}');
  buffer.writeln();
...
Future<Uint8List> gerarPdfBytes({
  ...
  final clienteFmt = comFallback(cliente);
  final cpfCnpjFmt = comFallback(cpfCnpj);
  final telefoneFmt = comFallback(telefone);
  final veiculoFmt = comFallback(veiculo);
  final total = itens.fold(0.0, (soma, item) => soma + item.valorTotal);
    ...
    pw.MultiPage(
      ...
      build: (context) => [
        ...
        pw.Container(
          ...
          child: pw.Column(
            ...
            children: [
              ...
              pw.Text('Nome: $clienteFmt',
                  style: const pw.TextStyle(fontSize: 9)),
              pw.Text('CPF/CNPJ: $cpfCnpjFmt',
                  style: const pw.TextStyle(fontSize: 9)),
              pw.Text('Telefone: $telefoneFmt',
                  style: const pw.TextStyle(fontSize: 9)),
        ...
        pw.TableHelper.fromTextArray(
          ...
          data: List<List<String>>.generate(itens.length, (i) {
            final item = itens[i];
            return [
              (i + 1).toString(),
              item.descricao,
              formatarQtdGlobal(item.quantidade),
              'R\$ ${valorFmtGlobal(item.valorUnitario)}',
              'R\$ ${valorFmtGlobal(item.valorTotal)}',
            ];
          }),
        ...
        pw.Align(
          ...
          child: pw.Container(
            ...
            child: pw.Text(
              'TOTAL GERAL: R\$ ${valorFmtGlobal(total)}',
              style: pw.TextStyle(
                fontSize: 12,
                fontWeight: pw.FontWeight.bold,
                color: PdfColors.white,
...
class _OrcamentoPageState extends State<OrcamentoPage> {
  ...
  void _abrirHistorico() async {
    final resultado = await Navigator.push<Map<String, dynamic>>(
      context,
      MaterialPageRoute(builder: (_) => const HistoricoPage()),
    );
    if (resultado != null) {
      _carregarOrcamentoParaEdicao(resultado);
    }
  }

  void _carregarOrcamentoParaEdicao(Map<String, dynamic> orcamento) {
    setState(() {
      _clienteController.text = orcamento['cliente'] as String? ?? '';
      _cpfCnpjController.text = orcamento['cpfCnpj'] as String? ?? '';
      _telefoneController.text = orcamento['telefone'] as String? ?? '';
      _veiculoController.text = orcamento['veiculo'] as String? ?? '';
      _itens.clear();
      final itensSalvos = (orcamento['itens'] as List)
          .map((e) => Map<String, dynamic>.from(e as Map));
      for (final item in itensSalvos) {
        _itens.add(OrcamentoItem(
          descricao: item['descricao'] as String,
          quantidade: (item['quantidade'] as num).toDouble(),
          valorUnitario: (item['valorUnitario'] as num).toDouble(),
        ));
      }
    });
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Orçamento carregado para edição.'),
  ...
  // ---------------------------------------------------------------------
  // FINALIZAR ORÇAMENTO: salva no histórico e limpa a tela pro próximo cliente
  // ---------------------------------------------------------------------
  Future<void> _finalizarOrcamento() async {
    if (_itens.isEmpty) {
      _mostrarErro('Inclua ao menos um item antes de finalizar.');
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Orçamento #$numero finalizado e salvo no histórico!'),
        backgroundColor: Colors.red.shade700,
      ),
    );
                ...
                return Padding(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        child: Text(
                          'CLIENTE: $_nomeCliente | ${qtd}x ${item.descricao} | R\$ ${valorFmtGlobal(item.valorTotal)}',
...
class _HistoricoPageState extends State<HistoricoPage> {
  ...
  Future<void> _persistir() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('zm_historico', jsonEncode(_historico));
    } catch (e) {
      debugPrint('Erro ao salvar histórico: $e');
    }
  }
  ...
  List<OrcamentoItem> _itensDoRegistro(Map<String, dynamic> orcamento) {
    return (orcamento['itens'] as List)
        .map((e) => Map<String, dynamic>.from(e as Map))
        .map((m) => OrcamentoItem(
              descricao: m['descricao'] as String,
              quantidade: (m['quantidade'] as num).toDouble(),
              valorUnitario: (m['valorUnitario'] as num).toDouble(),
            ))
        .toList();
  }
  ...
  Future<void> _excluirOrcamento(int index) async {
  ...
  void _verOrcamento(Map<String, dynamic> orcamento) {
    final itens = _itensDoRegistro(orcamento);
              ...
              ...itens.map((i) => Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Expanded(
                          child: Text(
                            '${formatarQtdGlobal(i.quantidade)}x ${i.descricao}',
                            style:
                                const TextStyle(color: ZenColors.textPrimary),
                          ),
                        ),
                        Text(
                          'R\$ ${valorFmtGlobal(i.valorTotal)}',
                          style: const TextStyle(
                              color: ZenColors.textSecondary),
                        ),
                      ],
                ...
                children: [
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        final bytes = await gerarPdfBytes(
                          ...
                          cliente: cliente,
                          cpfCnpj: orcamento['cpfCnpj'] as String? ?? '',
                          telefone: orcamento['telefone'] as String? ?? '',
                          veiculo: orcamento['veiculo'] as String? ?? '',
                          itens: itens,
                        );
                        final data =
                            DateTime.tryParse(orcamento['data'] as String) ??
                                DateTime.now();
                        final nomeArquivo =
                            'OS_${DateFormat('dd_MM_yyyy').format(data)}.pdf';
                        await Printing.sharePdf(
                            bytes: bytes, filename: nomeArquivo);
                      },
                      icon:
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: () async {
                        final texto =
                            gerarTextoWhatsApp(cliente: cliente, itens: itens);
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Orçamento #${item['numero']} — ${cliente.isEmpty ? 'Cliente não informado' : cliente}',
                            style: const TextStyle(
                              color: ZenColors.textPrimary,
                              fontWeight: FontWeight.bold,
                            ),
                          ...
                          const SizedBox(height: 4),
                          Text(
                            '${_formatarData(item['data'] as String)} • R\$ ${valorFmtGlobal(total)}',
                            style: const TextStyle(
                                color: ZenColors.textSecondary, fontSize: 13),
                          ),
                          const SizedBox(height: 8),
                          Row(
                            ...
                            children: [
                              TextButton.icon(
                                onPressed: () => _verOrcamento(item),
                                icon: const Icon(Icons.visibility_outlined,
                                    color: ZenColors.blue, size: 18),
                                label: const Text('Ver',
                                    style: TextStyle(color: ZenColors.blue)),
                              ),
                              TextButton.icon(
                                onPressed: () => _editarOrcamento(item),
                                icon: const Icon(Icons.edit_outlined,
                                    color: ZenColors.whatsapp, size: 18),
                                label: const Text('Editar',
                                    style:
                                        TextStyle(color: ZenColors.whatsapp)),
                              ),
                              TextButton.icon(
                                onPressed: () => _excluirOrcamento(index),
                                icon: const Icon(Icons.delete_outline,
